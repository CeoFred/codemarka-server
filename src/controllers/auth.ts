...
export const postLogin = (req: Request, res: Response, next: NextFunction) => {
    ...
    User.findOne({ email }, (err, userFound) => {
        ...
        if (err && !userFound) {
        ...
        else if (userFound && userFound.email) {
            userFound.comparePassword(password, (err, match) => {
                ...
                if (match) {
                    ...
                    return res.status(403).json(failed({ message: "Password is incorrect" }));
                }
            });
        } else {
...
export const postSignup = (req: Request, res: Response, next: NextFunction) => {
    ...
    User.findOne({ email }, (err, userFound) => {
        ...
        if (err) {
            ...
            return res.status(403).json(failed("email has already been taken"));
        } else {
            const user = new User({
                email,
                password,
